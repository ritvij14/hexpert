...
- [Hexpert Implementation (Vitest Foundation)](#hexpert-implementation-vitest-foundation)
...
## Hexpert Implementation (Vitest Foundation)

> The generic sections above are the philosophy. This section is the concrete
> Hexpert setup, layered on top. Last updated: 2026-06-27.

### Runner & commands

[Vitest 4](https://vitest.dev), auto-loaded from the root `vitest.config.ts`.
When `apps/web` gains tests, split this into `vitest.workspace.ts` with one
project per workspace (node here, jsdom for the frontend). From the repo root:

```bash
npm test              # vitest run (all projects, all layers)
npm run test:watch    # interactive watch
npm run test:coverage # with V8 coverage
```

### Layout — diverges from the co-location guidance above

The generic "Test File Structure" section recommends co-locating unit tests next
to the file under test. Hexpert does **not** do this for `@hexpert/agent`, because
`apps/agent/tsconfig.json` includes `src` and the Lambda build
(`npm -w @hexpert/agent run build:lambda`) runs `tsc` over `src`. Co-located
`*.test.ts` files (which import Vitest) would be compiled into the deployment
artifact and break the build. So all agent tests live under `apps/agent/test/`,
which `tsconfig` does not include:

```
vitest.config.ts                  # root config (node env, @hexpert/shared alias, setupFiles)
apps/agent/test/setup.ts           # env load + off-Lambda + ALLOWED_ORIGIN default
apps/agent/test/unit/             # pure-logic unit tests
apps/agent/test/integration/      # supertest against the Express app (no graph, no LLM)
apps/agent/test/eval/harness.ts   # parseSse + runTurn/runFlow + scorers (the eval library)
apps/agent/test/eval/harness.test.ts
apps/agent/test/flow/             # full graph flows (LLM + Redis mocked)
```

### Mock seams (external boundaries only)

Per the "Real paths over mocked shortcuts" principle, only the two external
boundaries are mocked in flow tests:

- **LLM** — `vi.mock` `apps/agent/src/utils/llmFactory.ts` (`createLLM`) to return
  LangChain's built-in `FakeListChatModel` (`@langchain/core/utils/testing`).
- **Redis checkpointer** — `vi.mock` `apps/agent/src/checkpointer.ts`
  (`getCheckpointer` / `initCheckpointer`) to return `MemorySaver`
  (`@langchain/langgraph`).

`vi.mock` factories are hoisted above imports and cannot reference imported
values, so `test/flow/qna.test.ts` uses `vi.hoisted` holders populated in
`beforeAll`. Reuse that pattern for new flow tests. Never mock the graph, the
route, the SSE writer, or `@hexpert/shared`.

### Environment loading

`apps/agent/test/setup.ts` is the global setup. It loads `.env` (repo root) and
`apps/agent/.env` (if present) into `process.env` with `override:false` — a real
shell/CI environment always wins and `.env` only fills gaps. It clears
`AWS_LAMBDA_FUNCTION_NAME` (tests always run "off-Lambda") and sets a default
`ALLOWED_ORIGIN` (the CORS middleware captures it at app import time). The
committed template is `.env.example`. In CI there is no `.env` file; the same
variables come from GitHub Secrets injected as env.

### App / bootstrap split

The Express app lives in `apps/agent/src/app.ts` (importable in ESM tests
without the `require.main === module` listen guard). `index.ts` imports `app`
and starts the server. Tests import `app` from `app.ts`; Lambda still boots
`index.ts` via `run.sh` (`node dist/index.js`) unchanged.

### Real-LLM evals (`evals/`)

Separate from the Vitest suite above. These drive the real Express app with the
**real LLM** (no mock), so they spend tokens and need BYOK keys + `REDIS_URL`.
They are on-demand only (`npm run eval` runs all four; `eval:router` /
`eval:audit` / `eval:qna` / `eval:wallet` run one each), never run by `npm test`,
and never run in CI. See `evals/README.md` for prerequisites,
cost, and design choices. Run `npm run eval:report` (port 4321) to browse all
trace files in `evals/.runs/` as a dark-mode dashboard (suite → case, with
pass/fail, latency, token usage, tool calls, and streamed answer). The deferred CI commit-gate layer (offline, mocked
LLM, runs on every push) and frontend tests are tracked in PRD §5 for future
reference; the existing `apps/agent/test/flow/qna.test.ts` covers the
plumbing-regression role ad hoc in the meantime.
<!-- Per-feature coverage notes go in docs/features/[feature].md → Testing section. -->
<!-- Add new patterns to this doc when a recurring test scenario type emerges that is not covered above. -->
